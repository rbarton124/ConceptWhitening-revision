...
def organize_train_split(src_dir, dst_dir, categories, samples_per_class, verbose):
    ...
    for category in tqdm(categories, disable=verbose < 1):
        ...
        images = [f for f in os.listdir(src_category_dir) if f.endswith(('.jpg', '.jpeg', '.png'))]

        if len(images) < samples_per_class:
            log(f"Warning: Not enough images for {category}. Found {len(images)}, needed {samples_per_class}",
                1, verbose)
...
def organize_val_split(val_dir, dst_dir, categories, samples_per_class, val_txt_file, categories_file, verbose):
    ...
    for category in tqdm(categories, disable=verbose < 1):
        ...
        if len(available_images) < samples_per_class:
            log(f"Warning: Not enough images for {category}. Found {len(available_images)}, needed {samples_per_class}",
                1, verbose)

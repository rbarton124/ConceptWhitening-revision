...
from MODELS.model_densenet_qcw import build_densenet_qcw, DenseNetQCW
...
parser.add_argument("--depth", type=int, default=161, help="DenseNet depth (161).")
...
# Feature toggles
parser.add_argument("--vanilla_pretrain", action="store_true", help="Train without Concept Whitening, i.e. vanilla DenseNet.") # used to work, isn't working lately TODO: ACTUALLY FIX [issue was subspace null mapping issue]
...
# Resume Checkpoint
def maybe_resume_checkpoint(model: nn.DataParallel[DenseNetQCW], optimizer: torch.optim.Optimizer, args):
    """
    Resume from checkpoint, handling DenseNet or QCW format.
    ...
    def rename_key(old_key):
        """
        Try a set of sensible renames for checkpoint keys so they match model_sd.
        Returns the first candidate that exists in model_sd, otherwise returns old_key
        (after stripping 'module.' if present).
        """
        # base: strip module. prefix
        base = old_key
        ...
        # 1) backbone prefix variants (if not already present)
        if not base.startswith("backbone."):
            candidates.append("backbone." + base)

        # 2) common DenseNet/torchvision suffix collapse: turn ".norm.1" -> ".norm1", ".conv.1" -> ".conv1"
        #    and apply with/without backbone prefix
        collapsed = re.sub(r'\.(norm|conv|relu|bn|weight|bias)\.(\d+)', lambda m: f".{m.group(1)}{m.group(2)}", base)
        if collapsed != base:
            candidates.append(collapsed)
            if not collapsed.startswith("backbone."):
                candidates.append("backbone." + collapsed)

        # 3) generic collapse any `.name.NUM.` -> `.nameNUM.` (heuristic)
        gen_collapsed = re.sub(r'\.(\w+)\.(\d+)\.', lambda m: f".{m.group(1)}{m.group(2)}.", base)
        if gen_collapsed != base and gen_collapsed not in candidates:
            candidates.append(gen_collapsed)
            if not gen_collapsed.startswith("backbone."):
                candidates.append("backbone." + gen_collapsed)

        # 4) common prefix fixes: module.classifier.* -> backbone.classifier.*
        if base.startswith("classifier.") and "backbone.classifier" in model_sd:
            candidates.append("backbone." + base)

        # 5) sometimes checkpoint uses features.* while model expects backbone.features.*
        if base.startswith("features.") and "backbone.features" in model_sd:
            candidates.append("backbone." + base)

        # 6) final conservative fallback: remove any leading "module." (already in base) and try that
        candidates.append(base)  # already added but safe
        ...
        # Return first candidate that exists in model_sd
        for c in final_cands:
            if c in model_sd:
                return c

        # nothing matched — return the original stripped-of-module key (base)
        return base
...
def train_epoch(train_loader, concept_loaders, model: nn.DataParallel[DenseNetQCW],
...
def align_concepts(model: nn.DataParallel[DenseNetQCW], subconcept_loaders, concept_dataset: ConceptDataset, batches_per_concept, lambda_):
    """
...
def compute_alignment_metrics(model: nn.DataParallel[DenseNetQCW], subconcept_loaders, concept_dataset: ConceptDataset, batches_per_concept, label_to_scname):
    """
...
def validate(loader, model: nn.DataParallel[DenseNetQCW], epoch, writer: SummaryWriter, mode="Val"):
    model.eval()
    criterion = nn.CrossEntropyLoss().cuda()
    losses = AverageMeter()
    top1   = AverageMeter()

...
class SingleLabelFolder(torch.utils.data.Dataset):
    """
    Loads **all** images found inside `root/` (recursively one level deep)
    and always returns dummy label 0.  Used when a concept folder contains
    images directly instead of sub-class directories.
    """
    ...
    def __init__(self, root: str, transform=None):
        ...
        self.samples = self._collect_files(root)

    def _collect_files(self, root):
        files = []
        for fname in os.listdir(root):
            p = os.path.join(root, fname)
            if os.path.isdir(p):                       # 1-level recursion
                files.extend([os.path.join(p, f)
                               for f in os.listdir(p)
                               if os.path.splitext(f)[1].lower() in self.IMG_EXTS])
            elif os.path.splitext(fname)[1].lower() in self.IMG_EXTS:
                files.append(p)
        if len(files) == 0:
...
def setup_dataloaders(args):
    ...
    for c_name in concept_names:
        ...
        # Heuristic: does c_path contain sub-directories (= old layout)?
        has_subdirs = any(os.path.isdir(os.path.join(c_path, x))
                          for x in os.listdir(c_path))

        if has_subdirs:
            c_dataset = datasets.ImageFolder(c_path, transform=train_transform)
        else:
            c_dataset = SingleLabelFolder(c_path, transform=train_transform)

...
from torch.utils.tensorboard import SummaryWriter  # <-- NEW for TensorBoard
...
# Global SummaryWriter so we don't change function signatures
writer = None
...
def main():
    ...
    if args.arch == "resnet_cw":
        if args.depth == 50:
            model = ResidualNetTransfer(
                365, args, [int(x) for x in args.whitened_layers.split(',')],
                arch='resnet50', layers=[3, 4, 6, 3],
        ...
        if args.depth == 50:
            model = ResidualNetBN(
                365, args, arch='resnet50', layers=[3, 4, 6, 3],
                model_file='./checkpoints/resnet50_places365.pth.tar'
            )
        if args.depth == 18:
            model = ResidualNetBN(
                365, args, arch='resnet18', layers=[2, 2, 2, 2],
                model_file='./checkpoints/resnet18_places365.pth.tar'
            )
        ...
        model = VGGBN(
            365, args, arch='vgg16_bn',
            model_file='./checkpoints/vgg16_bn_places365_12_model_best.pth.tar'
        )
    else:
        print("Warning: Architecture not recognized. Using default resnet18.")
        # fallback
        model = ResidualNetBN(
            365, args, arch='resnet18', layers=[2, 2, 2, 2],
            model_file='./checkpoints/resnet18_places365.pth.tar'
    ...
    conceptdir_test = os.path.join(args.concept_data, 'concept_test')
    ...
    for concept in concept_names:
        print(f"Loading concept dataset '{concept}'")
    ...
    if args.evaluate is None:
        ...
        for epoch in range(args.start_epoch, args.start_epoch + args.epochs):
            ...
            if args.arch == "resnet_cw":
                ...
                # For normal "resnet_original" or other arch without concept
                train(train_loader, [], model, criterion, optimizer, epoch)

            # evaluate on validation set
            prec1 = validate(val_loader, model, criterion, epoch)
            writer.add_scalar('Accuracy/val_top1', prec1, epoch)
        ...
        print(f"Training complete. Best Prec@1 = {best_prec1:.3f}")
        final_test_acc = validate(test_loader, model, criterion, epoch)
        print(f"Final Test Prec@1 = {final_test_acc:.3f}")
        writer.close()
    else:
        plot_figures(args, model, test_loader_with_path,
                     train_loader, concept_loaders, conceptdir_test)
        writer.close()
...
def train(train_loader, concept_loaders, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        writer.add_scalar('Train/Loss', losses.val.item(), iteration)
        writer.add_scalar('Train/Prec@1', top1.val.item(), iteration)
        writer.add_scalar('Train/Prec@5', top5.val.item(), iteration)
...
def train_baseline(train_loader, concept_loaders, model, criterion, optimizer, epoch, activation_mode='pool_max'):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        writer.add_scalar('TrainBaseline/Loss_main', losses.val.item(), iteration)
        writer.add_scalar('TrainBaseline/Loss_aux', loss_aux.val.item(), iteration)
        writer.add_scalar('TrainBaseline/Prec@1', top1.val.item(), iteration)
        writer.add_scalar('TrainBaseline/Prec_cpt@1', top1_cpt.val.item(), iteration)
...
def accuracy(output, target, topk=(1,)):
    ...
    # gather topk indices => shape is [batch_size, maxk]
